        ...
        for x, c in enumerate(line + "."):
            if c.isdigit():
                ...
                if new_num:
                    ...
                    new_num = ""
                if c != ".":
                    parts.append(Part(pos=Point(x, y), label=c))

    return parts, part_nums


def part1():
    parts, part_nums = parse()
    parts_dict = {p.pos: p for p in parts}

    def find_part(pn: PartNum):
        for dy in [-1, 0, 1]:
            for dx in range(-1, pn.size + 1):
                if Point(pn.pos.x + dx, pn.pos.y + dy) in parts_dict:
                    return True
        return False
...
def part2():
    parts, part_nums = parse()
    nums_dict: dict[Point, PartNum] = {}
    for pn in part_nums:
        for i in range(pn.size):
            nums_dict[Point(pn.pos.x + i, pn.pos.y)] = pn
    ...
    for part in parts:
        ...
        nums: set[PartNum] = set()
        for dy in [-1, 0, 1]:
            for dx in [-1, 0, 1]:
                p = Point(part.pos.x + dx, part.pos.y + dy)
                if p in nums_dict:
                    nums.add(nums_dict[p])

        if len(nums) == 2:
            first, second = list(nums)
...
.......................844.32......449..........932....................869....................77......................288..................."""  # noqa: E501
