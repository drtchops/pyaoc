...
def parse():
    ...
    for line in INPUT.splitlines():
        ident, reveals = line.split(": ")
        game = Game(id=int(ident[5:]), reveals=[])
        for reveal in reveals.split("; "):
            r = Game.Reveal()
            for cube in reveal.split(", "):
                num, color = cube.split()
                setattr(r, color, int(num))
            game.reveals.append(r)
        games.append(game)
    return games
...
Game 100: 16 blue, 12 red, 3 green; 2 green, 7 blue; 5 blue, 4 green; 10 blue, 6 red, 6 green; 5 red, 12 blue, 2 green; 9 red, 12 blue, 11 green"""  # noqa: E501
