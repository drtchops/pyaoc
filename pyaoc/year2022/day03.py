def priority(item: str):
    value = ord(item)
    return value - 96 if item.islower() else value - 38
...
def part2():
    ...
    for i, line in enumerate(INPUT.splitlines()):
        if i % 3 == 0:
            group_items = set(line)
        else:
            group_items = group_items.intersection(set(line))
        if i % 3 == 2:
            total += priority(list(group_items)[0])
    print(total)


INPUT = """NLBLfrNNLvqwbMfDqSjSzzSJjjggcdVs
